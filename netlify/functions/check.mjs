...
var check_default = async () => {
  const pub = process.env.VAPID_PUBLIC, priv = process.env.VAPID_PRIVATE;
  const mail = process.env.VAPID_SUBJECT || "mailto:notizen@example.com";
  if (!pub || !priv) {
    console.log("VAPID-Schl\xFCssel fehlen");
    return;
  }
  import_web_push.default.setVapidDetails(mail, pub, priv);
  const store = getStore({ name: "reminders", consistency: "strong" });
  const { blobs } = await store.list();
  const now = Date.now(), WINDOW = 10 * 60 * 1e3;
  let sent = 0;
  for (const b of blobs) {
    ...
    for (const it of rec.items || []) {
      if (it.at <= now && it.at > now - WINDOW) {
        try {
          await import_web_push.default.sendNotification(rec.sub, JSON.stringify({
            title: it.title,
            body: it.body,
            tag: it.id
          }));
          ...
          console.log("Push fehlgeschlagen:", err.statusCode);
        }
        changed = true;
      } else if (it.at > now - WINDOW) {
        keep.push(it);
      } else {
        changed = true;
      }
    ...
    if (changed) {
      if (keep.length) await store.setJSON(b.key, { ...rec, items: keep });
      else await store.delete(b.key);
    }
  }
  console.log("Erinnerungen verschickt:", sent);
};
var config = { schedule: "* * * * *" };
export {
  config,
